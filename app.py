...
# === Onglets ===
tab1, tab2, tab3 = st.tabs(["📌 Antibiotiques 2024", "🧪 Autres Antibiotiques", "🧬 Phénotypes Staph aureus"])
...
with tab1:
    ...
    df_ab = df_ab[df_ab["Week"].apply(lambda x: str(x).isdigit())]
    df_ab["Week"] = df_ab["Week"].astype(int)
    ...
    min_week, max_week = df_ab["Week"].min(), df_ab["Week"].max()
    week_range = st.slider("Plage de semaines", min_week, max_week, (min_week, max_week), key="range_ab2024")

    df_filtered = df_ab[(df_ab["Week"] >= week_range[0]) & (df_ab["Week"] <= week_range[1])]
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_filtered["Week"], y=df_filtered[selected_ab], mode='lines+markers', name=selected_ab))
    fig.add_trace(go.Scatter(x=df_filtered["Week"], y=[upper]*len(df_filtered), mode='lines', name="Seuil haut", line=dict(dash='dash')))
    fig.add_trace(go.Scatter(x=df_filtered["Week"], y=[lower]*len(df_filtered), mode='lines', name="Seuil bas", line=dict(dash='dot')))
...
with tab2:
    ...
    df_other = df_other[df_other["Week"].apply(lambda x: str(x).isdigit())]
    df_other["Week"] = df_other["Week"].astype(int)
    ...
    min_week, max_week = df_other["Week"].min(), df_other["Week"].max()
    week_range = st.slider("Plage de semaines", min_week, max_week, (min_week, max_week), key="range_other_ab")

    df_filtered = df_other[(df_other["Week"] >= week_range[0]) & (df_other["Week"] <= week_range[1])]
    values = pd.to_numeric(df_filtered[selected_ab], errors='coerce').dropna()
    q1, q3 = np.percentile(values, [25, 75])
    iqr = q3 - q1
    lower, upper = max(q1 - 1.5 * iqr, 0), q3 + 1.5 * iqr
